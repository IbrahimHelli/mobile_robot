...
const float secondTargetYaw = 90;  // İkinci ro tasyon hedefi
const float thirdTargetYaw = 180;   // Üçüncü rotasyon hedefi
const float fourthTargetYaw = 270;   // Dördüncü rotasyon hedefi
const float targetDistance = 500;   // mm cinsinden gidilecek ara mesafe
const float yawThreshold = 5;       // derece cinsinden sapma değeri
...
void loop() {
  ...
  calculateYaw(calibratedAccelX, calibratedAccelY, calibratedAccelZ, calibratedMagX, calibratedMagY, calibratedMagZ);

 // MPU6050 jiroskop verilerini oku ve kalibre et
  readGyroMPU6050(&rawGyroZ);
  getCalibratedGyro(rawGyroZ, &calibratedGyroZ); 

// ilk açıldığında açı düzenlene kadar 6 sanyiye çalış sonra açıyı referans al
while(flag==0){
...
void leftEncoderISR() {
  static unsigned long lastInterruptTime = 0;
  unsigned long currentTime = micros();
  if (currentTime - lastInterruptTime > 2000) {  // 1500 µs debounce süresi
    leftPulseCount++;
    lastInterruptTime = currentTime;
  }
}
...
void getCalibratedAccel(int16_t rawAccelX, int16_t rawAccelY, int16_t rawAccelZ, 
	...
	float temp[3];
	float accelOffset[3] =
	{ 125.057492, -12.818640,  -47.637789};

	float accelAinv[3][3] =
	{ { 1.000456,  0.00059, 0.001449},
...
void getCalibratedMag(int16_t rawMagX, int16_t rawMagY, int16_t rawMagZ, 
	...
	float temp[3];
	float magOffset[3] =
	{ -19.467699, 408.842280,  94.706312};

	float magAinv[3][3] =
	{ { 1.009179,  0.003661, 0.058009},
...
void initHMC5883L() {
   // Mag CNFG -->  Config A register adresi --> HMC5883L_REG_CONFIG_A ---- Örnekleme o
ranı: 8G range, 200Hz ölçüm modu -->  0x1C
...
void calculateYaw(float accelX, float accelY, float accelZ, float magX, float magY, float magZ) {
  ...
  yawMag = atan2(Ym, Xm) * 180.0 / PI; // negatif işaret pusula yönüne uygunluk için
